...
### Frontend
Location: the root project folder
...
- Consume backend APIs through a dedicated service layer in [src/services/api.ts](src/services/api.ts).
- Keep page-level logic separated from reusable UI components in [src/components](src/components).

Main frontend folders:
- [src/pages](src/pages) — route pages for each feature area.
- [src/components](src/components) — shared UI building blocks.
- [src/context](src/context) — authentication and shared state.
- [src/data](src/data) — mock/demo data used by the UI.
- [src/services](src/services) — API communication with the backend.
- [src/types](src/types) — shared TypeScript response types.
